...
public class CompraPanel extends javax.swing.JPanel {
    ...
    public CompraPanel() {
        ...
        compraTable.setEnabled(true);
        mtdc = new ComprasTableModel();
        compraTable.getSelectionModel().setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
        compraTable.setModel(mtdc);
        compraTable.setEnabled(true);

        //compraTable.getTableHeader().setDefaultRenderer(new EstiloTablaHeader());
        //compraTable.setDefaultRenderer(Object.class, new EstiloTablaFootRenderer());
        clientes = ClienteData.listCmb("");
        tiposComboxModel = new DefaultComboBoxModel(clientes.toArray());

        clienteCombo.setModel(tiposComboxModel);

        if (clientes.size() > 0) {
            //clienteSelected = clientes.get(0);
            //clienteCombo.getModel().setSelectedItem(clienteSelected);
     ...
     public void paintTable(ComprasTableModel tableModel) {
        this.mtdc = tableModel;
        compraTable.setModel(mtdc);

        compraTable.getColumnModel().getColumn(0).setMaxWidth(35);
        //compraTable.getColumnModel().getColumn(0).setCellRenderer(new DefaultTableCellRenderer() ); 
    ...
    private void setEventTable() {
        ...
        this.compraTable.getModel().addTableModelListener(tml);
    ...
    private void initComponents() {
        ...
        compraTable.setSelectionMode(javax.swing.ListSelectionModel.SINGLE_SELECTION);
        compraTable.setSelectionMode(javax.swing.ListSelectionModel.SINGLE_SELECTION);
        ...
        clienteCombo.addItemListener(new java.awt.event.ItemListener() {
            public void itemStateChanged(java.awt.event.ItemEvent evt) {
                clienteComboItemStateChanged(evt);
            }
        });
    ...
    private void registarClienteButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_registarClienteButtonActionPerformed
        // TODO add your handling code here:
            ...
            s.setIdCliente(clienteSelected.getId());
            s.setNomCliente(clienteSelected.getNombre());
            
            if (compraTable.getSelectedRow() != -1) {// ha seleccionado, update
                try {
                    Compra fila = (Compra) mtdc.getRow(compraTable.getSelectedRow());
                    s.setIdCompra(fila.getIdCompra());
                    System.out.println("id:" + s.getIdCompra());
                    if (s.getIdCompra()> 0) {
                        int returnId = CompraData.update(s);
                        if (returnId != 0) {
                            paintTable(new ComprasTableModel());
                            resetForm();
                ...
                try {
                    int returnId = CompraData.create(s);
                    if (returnId != 0) {
                        paintTable(new ComprasTableModel());
    ...
    private void clienteComboItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_clienteComboItemStateChanged
        // TODO add your handling code here:
         if (evt.getStateChange() == ItemEvent.SELECTED) {
            clienteSelected = (Cliente) clienteCombo.getSelectedItem();
            int id = clienteSelected.getId();
            if (id > 0) {
                // MsgPanel.success(" cliente seleccionado: " + clienteSelected.getId());
                List<Compra> productosDelTipo = CompraData.listActivesByCliente(clienteSelected.getId());
                if (productosDelTipo.size() > 0) {
                    int compraId = productosDelTipo.get(0).getIdCliente();// coge el primero
                ...
                clienteCombo.setBorder(new LineBorder(new java.awt.Color(0, 0, 0), 1));
                // MsgPanel.error("");
                paintTable(new ComprasTableModel());
            } else {
        ...
        tableHandlerEvent();
    }//GEN-LAST:event_clienteComboItemStateChanged
